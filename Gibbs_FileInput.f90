...
!	include "Tangent.inc"
	...
	character*256 aline
	integer*4 skipKFlu
	real*8 includeNew	! this is used in Gibbs, but not in GibbsMDF
...
10	continue
	write(12,*)fileHeader
	read(5,'(A64)')AssembTitle
	write(12,*)AssembTitle
...
!     CHOOSE SYSTEM COMPONENTS FOR THIS PROBLEM
	...
	read(5,*)(UseSysCo(i),i=1,numSysCoInFile)
	write(12,*)(UseSysCo(i),i=1,numSysCoInFile)
	read(5,*)IMASS
	iMass = 1		! Mass balance is required for MDF routines
	write(12,*)IMASS

!     Sort out the ones we need
	NC = 0
	do i = 1,numSysCoInFile
		if(UseSysCo(i).eq.1)then
			NC=NC+1
			NOEL(NC) = i		! NOEL is pointer to system components used.
			endif
		end do
!	We need pointers from the grain boundary phase components to the system component order
!	If no H2O is used, then the order is the same as i in NOEL
!	H2O is sysCo #11 in the data file (SPaC) and it is PhCo #1 in the grain boundary
	if(UseSysCo(11).eq.1)then
		! we are using H2O
		GB_to_Sys_Co(1) = nc
		do i = 2,nc
			GB_to_Sys_Co(i) = i-1
...
2225   continue
	...
	!if(ilong(11).eq.1)
! 	write(12,*)' Read VPstart(K),FRACTL(K),numOxygens(K)'
! 	read(5,*,END=2003)VPStart(K),FRACTL(K),numOxygens(K)
! 	write(12,*)VPStart(K),FRACTL(K),numOxygens(K)
	write(12,*)' Read VPstart(K),FRACTL(K),includeNew,numOxygens(K)'
	read(5,*,END=2003)VPStart(K),FRACTL(K),includeNew,numOxygens(K)
	write(12,*)VPStart(K),FRACTL(K),includeNew,numOxygens(K)
	if(numOxygens(k).eq.0)then
		call FSS_Alert('ALERT',' You forgot to include number of oxygens for this last phase')
		endif
...
2003  CONTINUE
	! set the variable GBdependentX to initial value
	GBdependentX = xPhCo(1,1)	! the dependent variable is always the first one
...
!     system components for this problem
	DO L=1,NC
		...
		!NumOxInOxide(L)=NumOxInOxideInFile(NOEL(L))
		CONAME(L)=CONameInFile(NOEL(L))
		do i = 1,NoCompPlotVars
			CompPlotCoeff(i,L) = CompPlotCoeffInFile(i,NOEL(L))
			end do
		end do

!	Scale all thermodynamic data and mineral formulas to have only 1 oxygen/phase component
! 	do kcur = 1,numPhMIF
! 		k = asmCurrent(kCur)
! 		! Add up the number of oxygens in each phase component
! 		sumOx = 0.0d0
! 		do j = 1,numPhCo(k)
! 			sumOx = sumOx + 
!	Actually, I'm going to put this on hold for now and, instead, use numOxygens(phase) to scale
!		reactions (Arx and MDF)
!		chemical potentials
...
!     set MPSTART,mp0,mp1 AND MP2 to initial values
	...
	!    		note: if this code is changed, it must also be changed in routine CHANGE
			...
			vmolStart(k) = vmol(k)			! this is a new variable (6/24/2023). 
								! We will use this value in Sub AdjustAsm to avoid cases where the 
								!   value of vmol(k) hasn't been calculated for this new assemblage
								! Using this value assumes that the molar volume of phases don't change during the calculations
								!  this is not strictly true (phases do change composition), but the impact is trivial
			end do
...
!	Note that the first phase component in every phase is dependent, so only numPhCo(k) - 1 are independent in each phase
      	NX=NP-numPh
...
!	common /CoTransCommon/a,b
	...
	j = 1
      	...
      	write(*,'(A16)')'Non multisite phase  ',phCoNameinFile(k,j),phasetype(K)
...
! 	write(*,*)compLine
! 	write(*,110)(coValue(j),j=1,numSysCoInFile)
110	format(40F6.2)
! 	pause ' Hit return to continue'
	return
	end
...
1000  format(2f8.1,9x,'Tstart and Pstart')
      WRITE(5,1010)(CoNameInFile(i),i=1,numSysCoInFile)
1010  format(40(A4,2x))
      WRITE(5,1001)(UseSysCo(i),i=1,numSysCoInFile)
...
!     (2)  volume, nfract and numOxygens variables
...
1002  	format(I6,21x,a32)
      	WRITE(5,1006)VP2(K),FRACTL(K),numOxygens(K)
...
1006  format(1x,E15.5,2I8,'     VStart,Fractl,numOxygens')
...
! *****************************************
	...
	fileName = './GibbsMDF_Essentials/'//trim(thmoFile)		
	close(2)
	close(1)
...
!     Read component names and molecular wts from thermo data file
	READ(1,*)numSysCoInFile
	write(*,*)' Reading component names, mol wts, cations in oxides, oxide to element weights'
      	read(1,*)(CONameInFile(J),J=1,numSysCoInFile)
      	read(1,*)(MOLWTInFile(J),J=1,numSysCoInFile)
      	read(1,*)(NumCatInOxideInFile(J),J=1,numSysCoInFile)
      	!read(1,*)(NumOxInOxideInFile(j),j=1,numSysCoInFile)
      	read(1,*)(OxToElWtInFile(J),J=1,numSysCoInFile)
...
!	read(1,*)dummy				! row of component labels
	...
	read(1,*)CompPlotName(i),CompPlotConst(i),(CompPlotCoeffInFile(i,j),j=1,numSysCoInFile)
2	continue
