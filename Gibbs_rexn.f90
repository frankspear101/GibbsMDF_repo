...
!     MATRIX
	...
	Do kCur = 1,numPh
		...
		do j = 1,numPhCo(k)
			...
			DO ll=1,NC
!				A(L,ll)=COMP(k,j,ll)
				A(L,ll)=COMP(k,j,ll)/numOxygens(k)	! This should scale the reaction
				end do
			end do
		end do
...
! c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
! c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
	Subroutine MDFequations
      	implicit none
!     	Routine to parse linearly independent reactions from subroutine REXN into an
!		independent set of MDF reactions
!		These MDF reactions are stored in array ArxMDF
...
! c*****************************************
      	Character VN3(maxPC)*4
	common/VN3names/VN3
	integer*4 i,ii,j,k,kcur,idep

!	Note that there are always only 3 phases in the assemblage.
!		The first phase is always the grain boundary phase and it is considered the "matrix" in MDF terminology
!			This GB phases contains all of the system components
!		The second and third phases are the MDF phases to be constrained by the parallel tangent algorithm
!			These MDF phases have the normal number of phase components 
!
...
!
!	The linearly independent reactions are stored like this at the bottom of the Arx array:
...
!		   -3.000  -2.000   0.000  -3.000   0.000   0.000   1.000
	numMDFrxn = 0
	ii = np-nrx+1			! this is the start of the linearly independent reactions
	idep = numPhCo(1) + 1		! index of dependent phase component for phase 1
	do 105 kcur = 2,numPh		! we skip the first phase because it is the grain boundary
	k = asmCurrent(kCur)
	if(numPhCo(k).eq.1)then		! phases with only 1 phase component do not have MDF equations
		ii = ii + 1
		idep = idep + 1
		go to 105
		endif		
!	if we are here, then the phase has more than one phase component
!	idep = ii			! index of independent phase component reaction
	do 110 j = 2,numPhCo(k)		! the first phase component is always the dependent one (e.g. Prp in above example)
	ii = ii + 1			! index of the independent reaction in the array Arx
	numMDFrxn = numMDFrxn + 1	! increment number of MDF reactions
	do 110 i = 1,np			! loop through every phase component
	ArxMDF(numMDFrxn,i) = Arx(ii,i) - Arx(idep,i)
110	continue	
	idep = idep + numPhCo(k)	! Move to the next phase and set dependent phase component	
	ii = ii + 1			! index of the independent reaction in the array Arx
105	continue
!	Output results	
	if(iLong(7).eq.1)then
		write(12,*)'  '
		write(12,*)' Number of MDF equations (numMDFrxn) = ',numMDFrxn
		write(12,210)(VN3(J),J=1,NP)
210     	FORMAT(' ',4X,50(A4,4X))
		do i = 1,numMDFrxn	
			write(12,220)(ArxMDF(I,J),J=1,NP)
220     		FORMAT(' ',50F8.3)
			end do
		endif
	!NEQ = numMDFrxn
