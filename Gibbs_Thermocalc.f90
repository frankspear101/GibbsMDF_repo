...
! 		go to 9000
	...
	case default
		write(*,*)' The PhaseType(k) was not correct (no code found)'
...
9900  continue
	if(iLong(1).eq.1)then
		...
		WRITE(*,*)k,phname(k)
		write(*,'(20A12,2x)')(phCoName(k,i),i=1,numPhCo(k))
		write(*,'(20E14.4)')(xPhCo(k,i),i=1,numPhCo(k))
		write(*,*)'Tc,Pb= ',TC,PB
...
!     		Activities
		...
		if(iflag.eq.1)then
			write(*,*)'Activity out of bounds first call to Activity() min = ',k,phName(k),TC,TK,PB
			go to 9900
			endif
...
!	called from case(2) in dudPTX
      implicit none
      integer*4 AorB
      real*8 Tref,TL1bar,DTDPslope,L1,L2,TLatP,Pbars,TK,T9
      real*8 x1,x2,x3,x4,Td,Tr,Hlambda,Slambda,Glambda
      data Tref,TL1bar,DTDPslope,L1,L2/373.0D0,848.0D0,0.023743D0,&
     &-0.09186959,0.00024607D0/
